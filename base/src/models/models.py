...
class DialogManager:
    ...
    def lock_for(self, user_id: int, chat_id: int) -> asyncio.Lock:
        """Per-chat lock: serializes concurrent turns, prevents 400 races."""
...
class TelegramDialogManager(DialogManager):
    ...
    async def reply_on_text(
        ...
        async with self.lock_for(uid, cid):
            chat = await self.get_or_create_chat(uid, cid)
